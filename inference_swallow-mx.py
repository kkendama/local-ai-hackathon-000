...
model_name = "aixsatoshi/Swallow-MX-8x7b-NVE-chatvector-Mixtral-instruct"
...
example1_assistant = """指示文: 1週間後に台風が首都圏を直撃するようなのですが、その日はどうしても外せない予定があり、外出をすることになっています。
...
example2_assistant = """指示文: 寒さが厳しくなってきました。暖房費を抑えるために、省エネで効率的な暖房方法を知りたいです。"""
...
example3_assistant = """指示文: ゴルフクラブを購入しようと思っているのですが、スタンド型とカート型のどちらを選ぶべきでしょうか？
それぞれのメリットとデメリットについて教えてください。
また、初心者はどちらを選ぶべきか、詳しい方の意見を伺うことは可能でしょうか？"""

def generate_prompt(text):
    prompt= f'<s>[INST] {example1_user} [/INST] {example1_assistant}</s> [INST] {example2_user} [/INST] {example2_assistant}</s> [INST] {example3_user} [/INST] {example3_assistant}</s> [INST] 次の文章を読んで、AIへの指示文を考えてください。\n{text} [/INST] 指示文: '
    return prompt
...
def chat(prompt):
    ...
    messages = [
        ...
        {"role": "user", "content": "次の文章を読んで、AIへの指示文を考えてください。\n" + text},
    ]
